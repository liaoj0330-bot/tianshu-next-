...
test("today read model keeps a truthful empty state without demo data", async () => {
  ...
  try {
    ...
    assert.deepEqual(today.protected_projects, []);
...
test("today read model is SQLite-driven, human-readable, and separates protected projects", async () => {
  ...
  try { const today=await fetch(`http://${address.address}:${address.port}/v1/today`).then(r=>r.json()); assert.equal(today.state_authority,"sqlite"); assert.equal(today.focus.project_key,"tianshu"); assert.equal(today.projects.length,1); assert.equal(today.protected_projects.length,1); assert.equal(today.confirmations[0].result.interaction.state_candidate.decision_card.changes[0].label,"今日精力状态"); assert.deepEqual(today.confirmations[0].effects,["会更新奈奈的正式状态","不会创建任务","不会派发 Agent"]); } finally { await gateway.close(); db.close(); rmSync(root,{recursive:true,force:true}); }
});