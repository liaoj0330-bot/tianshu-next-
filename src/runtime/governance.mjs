...
export function finishJob(db, jobId, outcome, detail = {}) {
  const job = db.prepare("SELECT * FROM jobs WHERE job_id=?").get(jobId); if (!job) throw new Error("unknown job");
  return tx(db, () => { const success = outcome === "succeeded"; const next = success ? "succeeded" : (job.attempts < job.max_attempts ? "retry_wait" : "failed"); db.prepare("UPDATE jobs SET status=?,available_at=?,updated_at=? WHERE job_id=?").run(next, success ? now() : iso(100), now(), jobId); if (job.lease_id) { db.prepare("UPDATE worker_leases SET status='released' WHERE lease_id=?").run(job.lease_id); db.prepare("DELETE FROM project_locks WHERE lease_id=?").run(job.lease_id); } if (!success) db.prepare("INSERT INTO failure_cases VALUES (?, ?, ?, ?, ?, ?)").run(newId("failure"), jobId, job.project_id, detail.code ?? "execution_failed", canonicalJson(detail), now()); appendEvent(db,"job",jobId,`job.${next}`,detail); return next; });
}
